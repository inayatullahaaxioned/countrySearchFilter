...
//showing all country first
countryList.forEach((country) => {
    const li = document.createElement('li');
    li.className = 'country-name all';
...
form.addEventListener('input', (e) => {
    e.preventDefault();
    const result = [];
    const countryList = document.querySelectorAll('.country-name.all');
    // ul.innerHTML = "";
    countryList.forEach((country) => {
        const countryName = country.querySelector('.name');
        if (countryName.innerText.toLowerCase().match(input.value.toLowerCase())) {
            result.push(countryName.innerText);
...
toggle.addEventListener('click', () => {
    ...
    names.forEach((name) => {
        if (circle.classList.contains('active')) {
            const checkbox = name.querySelector('.checkbox');
            if (!checkbox.checked) {
                name.classList.add('hide')
                name.classList.remove('all')
            }
        } else {
